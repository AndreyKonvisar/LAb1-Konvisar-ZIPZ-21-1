    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                while (!endGame)
                {
                    ...
                    if (endGame)
                    {
                        ...
                        else
                        {
                            ...
                            winResult($"{winPlayer.getPlayerName()} Ти виграв!");
                            winPlayer.addWin();
                        }
                ...
                Console.Write("Хочеш спробувати ще раз? (y/n): ");
            ...
            {
                winResult($"{player1.getPlayerName()} Це переможець!");
            }
            else if (player2.getCountWin() > player1.getCountWin())
            {
                winResult($"{player2.getPlayerName()} Це переможець!");
            }
            else
            {
                winResult("ПЕРЕМОГЛА ДРУЖБА!:)");
